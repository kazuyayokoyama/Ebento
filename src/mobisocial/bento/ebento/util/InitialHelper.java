...
public class InitialHelper {
	...
	public Musubi initMusubiInstance(boolean bAsync) {
		...
		try {
			PackageInfo packageInfo = mActivity.getPackageManager().getPackageInfo(
					"mobisocial.bento.ebento", PackageManager.GET_META_DATA);
	...
	public void goMarket() {
		// Go to Market
		Uri uri = Uri.parse("market://details?id=mobisocial.bento.ebento");
		Intent intent = new Intent(Intent.ACTION_VIEW, uri);
		mActivity.startActivity(intent);
		mActivity.finish();
	}

	private class EventAsyncTask extends AsyncTask<Void, Void, Boolean> {
		...
		public EventAsyncTask(Musubi musubi, Context context, int versionCode) {
			mMusubi = musubi;
			mContext = context;
